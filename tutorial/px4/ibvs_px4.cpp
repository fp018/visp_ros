/****************************************************************************
 ...
 int main(int argc, char **argv)
 {
   try {
    ...
    ros::NodeHandle _n;
    ros::Publisher point_pub = _n.advertise<geometry_msgs::PoseStamped>("/drone/camera/tag_pose", 0);
   ...
   // ros::Subscriber ee_sub = _n.subscribe("/drone/ee_pos_corner", 1000, ee_pointsCallback);
    ...
    if(true){
      ...
      //  g.setCameraInfoTopic( "/usb_cam/camera_info" );
       ...
       //vpDisplayOpenCV display( I, orig_displayX, orig_displayY, "Color image" );
       vpDisplayX display;
       ...
       // double lambda = 0.5;
       //vpAdaptiveGain lambda = vpAdaptiveGain(1.5, 0.7, 1);  //vpAdaptiveGain lambda( 4, 1.2, 25 ); 
       vpAdaptiveGain lambda = vpAdaptiveGain( 1.5, 0.7, 1 );
       //vpAdaptiveGain lambda =  vpAdaptiveGain( 4, 1.2, 25 ); 
        ...
        */
       vpRxyzVector c1_rxyz_c(vpMath::rad(-0.0), vpMath::rad(0), 0);
       ...
       //vpTranslationVector c1te(0, 0.00, -0.08);             // Translation between (c1) and (e)
       vpRotationMatrix c1Re{0, -1, 0, -1, 0, 0, 0, 0, -1};
       vpTranslationVector c1te(0, 0.00, 0.00); 
       ...
       vpHomogeneousMatrix cMe = c1Mc.inverse() * c1Me;
       ...
       while (ros::ok() && runLoop) {
         ...
         if (detector.getNbObjects() != 0) {
            ...
            pose.pose.position.x = T_detected(0,3);
            pose.pose.position.y = T_detected(1,3);
            pose.pose.position.z = T_detected(2,3);
            pose.pose.orientation.x = quat_detected(1);
            pose.pose.orientation.y = quat_detected(2);
            pose.pose.orientation.z = quat_detected(3);
            pose.pose.orientation.w = quat_detected(0);
